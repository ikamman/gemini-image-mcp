    ...
    fn validate(&self, source: &String) -> McpResult<()> {
        if source.trim().is_empty() {
            return Err(McpError::InvalidInput(
                "Image source cannot be empty".to_string(),
            ));
        }

        if source.len() > 2048 {
            return Err(McpError::InvalidInput(
                "Image source URL/path too long (max 2048 characters)".to_string(),
            ));
        }
...
impl Validator<String> for OutputPathValidator {
    fn validate(&self, path: &String) -> McpResult<()> {
        ...
        if path.len() > 2048 {
            return Err(McpError::InvalidInput(
                "Output path too long (max 2048 characters)".to_string(),
            ));
        }

        if path.contains("..") {
            return Err(McpError::InvalidInput(
                "Path traversal not allowed in output path".to_string(),
            ));
        }
        ...
        if !has_valid_extension {
            return Err(McpError::InvalidInput(format!(
                "Unsupported output file extension. Allowed: {}",
        ...
        if let Some(parent) = path_obj.parent() {
            ...
            if !parent.as_os_str().is_empty() && !parent.exists() {
                return Err(McpError::FileSystemError(format!(
...
mod tests {
    ...
    fn test_image_source_validator() {
        ...
        assert!(
            validator
                .validate(&"https://example.com/image.png".to_string())
                .is_ok()
        );
        ...
        assert!(
            validator
                .validate(&"../../../etc/passwd".to_string())
                .is_err()
        );
        assert!(
            validator
                .validate(&"http://insecure.com/image.jpg".to_string())
                .is_err()
        );
    ...
    fn test_get_mime_type_from_extension() {
        ...
        assert_eq!(
            get_mime_type_from_extension("/path/to/image.png"),
            "image/png"
        );
        assert_eq!(
            get_mime_type_from_extension("./relative/path.webp"),
            "image/webp"
        );
    }
    ...
    fn test_output_path_validator() {
        ...
        assert!(
            validator
                .validate(&"../../../etc/passwd.jpg".to_string())
                .is_err()
        );
        assert!(validator.validate(&"image.txt".to_string()).is_err());
        assert!(
            validator
                .validate(&"/nonexistent/directory/image.jpg".to_string())
                .is_err()
        );
