...
use crate::validation::{Validator, PromptValidator, OutputPathValidator, ImageSourceValidator};
use reqwest;
...
impl GeminiClient {
    pub fn new(api_key: String) -> McpResult<Self> {
        if api_key.trim().is_empty() {
            return Err(McpError::AuthenticationError("API key is empty".to_string()));
        }
    ...
    pub async fn analyze_image(&self, input: &AnalyzeImageInput) -> McpResult<String> {
        let user_prompt = input.user_prompt.as_deref().unwrap_or("Caption this image.");
        ...
        let (mime_type, encoded_image) = self.image_service
            .fetch_and_encode(&input.image_source)
            .await
            .map_err(|e| {
                error!("Failed to fetch and encode image '{}': {}", input.image_source, e);
                e
            })?;
        ...
        if !response.status().is_success() {
            ...
            return match status.as_u16() {
                ...
                429 => Err(McpError::RateLimitError("Gemini API rate limit exceeded".to_string())),
        ...
        if let Some(error) = gemini_response.error {
            error!("Gemini API returned error: {} - {}", error.code, error.message);
    ...
    pub async fn generate_image(&self, input: &GenerateImageInput) -> McpResult<String> {
        ...
        let response = self.client
            ...
            .await
            .map_err(|e| {
                error!("Failed to send request to Gemini API for image generation: {}", e);
                e
            })?;
        ...
        let image_bytes = general_purpose::STANDARD.decode(&base64_image_data)
            .map_err(|e| {
        ...
        fs::write(&input.output_path, &image_bytes).await
            .map_err(|e| {
    ...
    pub async fn edit_image(&self, input: &EditImageInput) -> McpResult<String> {
        ...
        let response = self.client
            ...
            .await
            .map_err(|e| {
                error!("Failed to send request to Gemini API for image editing: {}", e);
                e
            })?;
        ...
        fs::write(&input.output_path, &image_bytes).await
            .map_err(|e| {
                error!("Failed to write edited image to '{}': {}", input.output_path, e);
            ...
            if let Some(content) = candidate.get("content") {
                if let Some(parts) = content.get("parts").and_then(|p| p.as_array()) {
                    ...
                    for part in parts {
                        ...
                        if let Some(inline_data) = part.get("inline_data").or_else(|| part.get("inlineData")) {
